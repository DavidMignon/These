...
Ici, $A_i$ est la surface exposée accessible au solvant de l'atome $i$.$\sigma_i$ est un paramètre qui représente la préférence de chaque atome à être exposé ou caché du solvant. Les atomes du soluté sont divisés en quatre groupes avec pour chacun une valeur $\sigma_i$ spécifique en $cal/mol/Å^2$.Nous travaillons avec deux jeux différents pour les $\sigma_i$. Le premier et un jeu déjà utilisé dans differentes études (\cite{Mignon16},\cite{Druart16b}):
non polaire -5,aromatique -40,polaire -80,ionique -100.
et le second proviennent d'une étude plus récente de notre laboratoire, dans laquelle il a été optimizé
sur un ensemble de proteins PDZ \ref{Gaillard17}:

??????

On attribue aux atomes d'hydrogène un coefficient de surface de 0. Les surfaces sont calculées par l'algorithme de Lee et Richards \cite{Lee71},  qui est implémenté dans le programme XPLOR \cite{Xplor}. Les simulations MC utilisent une constante diélectrique pour la protéine $\epsilon_p = 4$ où $8$ (voir la partie Résultats).
...
Nous utilisons deux variantes du modèle  GB, la méthode NEA pour \og Nativce Environment Approxition\fg et la méthode FDB \og Fluctuating Dielectric Boundary\fg \cite{Villa01}.Elles sont présentées respectivement en \ref{NEA} et \ref{FDB}.
