...
\end{equation}
où $n$ est périodicité de la rotation, $A_{i,n}$ est la constante de torsion, $\Phi_i$ l'angle dièdre, c'est-à-dire pour 4 atomes $a_1$, $a_2$, $a_3$ et $a_4$, reliés par 3 liaisons $a_1-a_2, a_2-a_3$ et $ a_3-a_4$, l'angle formé par les plans $(a_1,a_2$,$a_3)$ et $(a_2,a_3$,$a_4)$, $\Phi^0$ est la phase. L'énergie de déformation des angles impropres $E_{impr}$ exprime la déformation d'un  ensemble  de 4 atomes  par rapport à une conformation planaire ou tétraédrique. Pour un atome $a_1$ relié à 3 atomes $a_2$,$a_3$ et $a_4$, elle a la forme:
\begin{equation}
...
\begin{equation}
  \forall n \geqslant 0 , \forall (i_0,i_1,...,i_{n-1},i) \in E^{n+1}, P(X_n=i|X_{n-1}=i_0,X_{n-2}=i_1,..,X_{1}=i_{n-2},X_{0}=i_{n-1}) = P(X_n=i|X_{n-1}=i_0) 
\end{equation}
...
Soit $q$ une probabilité sur $E$, une chaîne de Markov est dîte réversible par rapport à $q$ si
...
\end{equation}
Ainsi, la probabilité $q$ avant la transition $P$ vaut la probabilité après, la chaîne est dîte stationnaire pour $q$. Il nous faut alors construire une chaîne de Markov réversible pour la cible \pi. Mais cela une suffit pas, en effet, même si une distribution stationnaire est connue, rien ne dit que la chaîne va entrer dedans. 
...
\begin{equation}
\pi^B(i) = \frac{1}{\sum_{j \in E}\exp(\frac{E_j}{kT})} \exp(\frac{E_i}{kT})
\end{equation}
...
\end{equation}\\
avec $\Delta E =  E_j - E_i$. Ainsi,le calcul de la fonction de partition est évité!
...
Si l'on injecte cette nouvelle probabilité dans ~\ref{balance}, on a encore le respect de la balance détaillée et la convergence. L'algorithme est résumé en \ref{alg:Metro}. Dans toute la suite, l'algorithme Monte Carlo (MC) désigne l'algorithme de Metropolis-Hastings, ce qui est un usage courant.
...
On considère $N$ simulations MC du système à $N$ différentes températures. Ces répliques du système sont indépendantes les unes par rapport aux autres. On note $T^m$ avec $m=1,..,N$ les températures. Toutes ces températures sont différentes et il y a toujours une réplique à chaque température. Ainsi, nous nous plaçons dans un ensemble généralisé noté $E^N$, constitué des $N$-uplets $(x^1,...,x^n)$ avec les $x^i$ éléments de $E$, et $1 \leq i \leq N$  indexant les répliques. On travaille avec une chaîne de Markov $\{X_t\}_{1<t}$ , avec $X_t=(x^1_t,...,x^n_t)$ une variable aléatoire à valeur dans $E^N$. On ajoute maintenant, un nouveau type de déplacement, celui qui consiste a intervertir au temps $t$ l'état $x^i$ de la simulation à la température $T^i$ avec l'état $x^{i+1}$ à la température $T^{i+1}$. On a alors:
...
\end{equation}\\
En injectant cette expression de $\pi^B$ et ~\ref{REMC_move} dans \ref{bd_Metropolis} on a:
\begin{equation}
\frac{acc_{swap}(X_{t+1}|X_t)}{acc_{swap}(X_t|X_{t+1})} =\frac{\pi^B(X_t)}{\pi^B(X_{t+1})}  = \frac{ -\exp(\frac{-E_{x^i}}{kT^i}) \exp(\frac{-E_{x^{i+1}}{kT^{i+1}})}{\exp(\frac{-E_{x^{i+1}}}{kT^i}) -\exp(\frac{E_{x^i}}{kT^{i+1}})} = \exp(-\Delta) 
\end{equation}\\
avec $\Delta = (\frac{1}{kT^i} -\frac{1}{kT^j})(E_{x^i} - E_{x^{i+1}})$ \\
Ceci peut être satisfait par le critère de Metropolis:

si
$\Delta >0$ alors $acc_{swap} (X_{t+1}|X_t)} = \exp(- \Delta)$ \\
sinon 
$acc_{swap} (X_{t+1}|X_t)} = 1$\\
L'algorithme REMC peut alors se décrire comme en \ref{algoREMC}
